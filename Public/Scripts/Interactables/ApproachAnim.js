...
var minManipDist = script.getSceneObject().getComponent("Component.ManipulateComponent").minDistance;
var maxManipDist = script.getSceneObject().getComponent("Component.ManipulateComponent").maxDistance;
...
function onLensTurnOff() {
    if (script.api.animMixer) {
        ...
        if (script.api.animMixer.getLayer(script.approachLoopAnimLayer)) {
            script.api.animMixer.getLayer(script.approachLoopAnimLayer).weight = 0.0;
        }
    ...
    if (global.scene.getCameraType() == "back" && !approachAnimPlaying) {
        if (script.approachStartAnimLayer && script.approachStartAnimLayer != "" && script.api.animMixer) {
            ...
            if (script.api.animMixer.getLayer(script.approachStartAnimLayer)) {
                ...
                script.api.animMixer.getLayer(script.approachStartAnimLayer).weight = 1.0;
            }
