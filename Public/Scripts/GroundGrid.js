...
function onFrameUpdated(eventData) {
    ...
    if (isTouching) {
        groundMeshVisual.mainMaterial.mainPass.mainColor = vec4.lerp(curColor, new vec4(1, 1, 1, 1), script.fadeUpSpeed);
    } else {
        groundMeshVisual.mainMaterial.mainPass.mainColor = vec4.lerp(curColor, new vec4(0, 0, 0, 1), script.fadeDownSpeed);
    }
